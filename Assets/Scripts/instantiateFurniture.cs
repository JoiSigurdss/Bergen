 ...
 * 
 */
public class instantiateFurniture : MonoBehaviour 
{
	...
	void CreateRandomPrefab()
	{
		...
		GameObject newPrefab = Instantiate (prefab, transform.position + new Vector3 (0, 0.5f, 0), transform.rotation) as GameObject;
		//GameObject newPrefab = Instantiate (prefab, transform.position + new Vector3 (0, 0.5f, 0), transform.rotation) as GameObject;
	...
	void OnGUI () 
	{
		...
		if (GUI.Button (new Rect (buttonPositionX,0,150,50), "Create " + prefab.name)) 
			CreateRandomPrefab();

		// Button to delete all data.
		if (GUI.Button (new Rect (buttonPositionX,50,150,50), "Delete all "+ prefab.name))
		{
			// Delete entire file.
			ES2.Delete(filename);
